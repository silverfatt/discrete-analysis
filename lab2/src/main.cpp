    ...
    while (std::cin >> command) {
        if (command == "+") { // Add new node
            ...
            std::transform(word.begin(), word.end(),
                           word.begin(), tolower);
            tree->Delete(word);
        } else if (command == "!") { // Work with files: Save or Load
            std::string option;
            std::cin >> option;
            if (option == "Save") {
                std::cin >> word;
                std::ofstream file(word, std::ios::binary | std::ios::out);
                tree->Save(tree->root, file);
            } else {
                std::cin >> word;
                std::ifstream file(word, std::ios::binary | std::ios::in);
                tree->ClearTree();
                tree->Load(file);
                std::cout << "OK\n";
            }
        } else if (command == "%") { // Debugging...
            //tree->PrintTree(tree.);
